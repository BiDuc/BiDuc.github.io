// [052223HG] Add class "active" when click for 2 CTA
$(".cta-btn > a").click(function(){
	$(this).parents(".cta-btn").find("a").removeClass("active");
	$(this).addClass("active");
});

// [052220HG] Add 2 CTA button for 4 tab " Character, Shoes, Bicycle, Swimwear "
function ShowItems($target, $itemID){
	$('#nft-items-menu').children().removeClass('active');
	$($target).addClass('active');

	$('#nft-items-container').find('.slider-container').removeClass('active-tab');
	$('#nft-items-container').find('.slider').removeClass('active-slider');
	$('#' + $itemID).addClass('active-tab');
	$('#' + $itemID).find('.slider').addClass('active-slider');
	$('#' + $itemID).find('.slider').css("position", "inherit");
	$('#' + $itemID).find('.slider').css('opacity', '1');	

	$('#nft-items-container').find('.flickity-enabled').hide();
	$('#' + $itemID).find('.slider').show();
...
function ShowAllNFTItems(){
	...
	$('#nft-items-container').find('.slider').css("position", "absolute");
	$('#nft-items-container').find('.slider').show();
	$('#nft-items-container').find('.active-slider').css("opacity", "1");
	$('#nft-items-container').find('.active-slider').css("position", "inherit");
...
function ShowAllCardSliders(){
	if ($(window).width() <= 991) {
		$('.card-container').css("position", "absolute");
		$('.card-container-mb').css("position", "inherit");
	}
	else{
		$('.card-container').css("position", "inherit");
		$('.card-container-mb').css("position", "absolute");
	}
		...
		const $carousel = $(".about-section .about-slider .slider").flickity({
			...
			on: {
				ready: function() {			   
					// [052223HG]  	
					InitNFTItems();
					if($(this.element).find('.slider-item').length < 3){
						$(this.element).addClass('default-slider-height');
					}
				},
...
$('.toggle-tab').click(function(){
    var $target = $(this).parents('.slider-container').find($(this).attr('target'));
    var delay = parseFloat($target.css('transition-duration'))*1000;
    $(this).parent().addClass('active').siblings().removeClass('active');
    $target.siblings('.tab-pane').removeClass('in');
    setTimeout(function(){
